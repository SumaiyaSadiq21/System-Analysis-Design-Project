<?php
 ...
 if(isset($_POST["add_to_cart"]))
 {
      if(isset($_SESSION["shopping_cart"]))
      {
         ...
         if(!in_array($_GET["Component_Number"],$item_array_id))
         {
              $count = count($_SESSION["shopping_cart"]);
              $item_array = array(

               'item_Medicine_number' => $_GET["Component_Number"],
     ...
     $_SESSION["shopping_cart"] [$count] =$item_array;
         ...
         else
         {
           ...
           echo '<script>window.location="viewcomponents.php"</script>';
         }
 ...
 if(isset($_GET["action"]))
 {
      if($_GET["action"] == "delete")
      {
           ...
           {
                if($values["item_id"] == $_GET["Component_Number"])
                {
...
<?php
$query= "SELECT * FROM medical_components ORDER BY Component_Number ASC" ;
...
if(mysqli_num_rows($result) > 0)
{
    while ($row = mysqli_fetch_array($result))
    {
        ...
        ?>
        <div class= "col_md-4">
            <form method= "post" action ="viewcomponents.php?action=add&Component_Number=<?php echo $row["Component_Number"]; ?>">
            <div style="border:1px solid #333; background-color:#f1f1f1; border-radius:5px; padding:16px; width:300px;" align="center">
            <img src="<?php echo $row["Component_Image"]; ?>"width="100" height="120" />
            <h4 class="text-info"><?php echo $row ["Component_Name"]; ?></h4>
            <h4 class="text-danger">Tk <?php echo $row ["Cost"]; ?></h4>
            <input type="text" name="quantity" class="form-control" value="1" />
            <input type="hidden" name="hidden_name" value="<?php echo $row["Component_Name"]; ?>"
            />  
    ...
    <?php
     ...
     <div class="table-responsive">  
          <table class="table table-bordered">  
               ...
               ?>  
               <tr>  
                    <td><?php echo $values["item_Medicine_Name"]; ?></td>  
                    <td><?php echo $values["item_quantity"]; ?></td>  
                    <td>Tk <?php echo $values["item_Price"]; ?></td>  
                    <td>Tk <?php echo number_format($values["item_quantity"] * $values["item_Price"], 2); ?></td>  
                    <td><a href="viewcomponents.php?action=delete&id=<?php echo $values["item_Medicine_number"]; ?>"><span class="text-danger">Remove</span></a></td>  
               </tr>  
               <?php  
          ...
          <input type="submit" name="proceedpayment" class="btn btn-primary" value="Proceed Payment" /> 
     </div>  
     </div>  
           <br />  
